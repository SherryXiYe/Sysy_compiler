...
MachineOperand::MachineOperand(int tp, double val)
{
    this->type = tp;
...
{
    //添加SREG判断，如果是浮点寄存器，则输出为s..
    if(type == MachineOperand::SREG){
...
LoadMInstruction::LoadMInstruction(MachineBlock* p,
                    MachineOperand* dst, SymbolEntry* se, int cond)
{
    this->parent = p;
    this->type = MachineInstruction::LOAD;    
    this->op = -1;
    ...
    this->def_list.push_back(dst);
    this->use_list.push_back(fp_reg);
    dst->setParent(this);
    fp_reg->setParent(this);
}
...
void LoadMInstruction::output()
{
    if (se){
        ...
        auto src2 = new MachineOperand(MachineOperand::IMM, offset);
        if (offset < -255 || offset > 255){
            auto internal_reg = new MachineOperand(MachineOperand::REG, 3);
            auto cur_inst = new LoadMInstruction(nullptr, internal_reg, src2);
            cur_inst->output();
            use_list.push_back(internal_reg);
        }
        else{
...
//调整r0,r1,r2,r3偏移顺序
StoreMInstruction::StoreMInstruction(MachineBlock* p, 
        MachineOperand* src1, SymbolEntry* se, bool isParam, int cond)
{
    this->parent = p;
    this->type = MachineInstruction::STORE;    
    this->op = -1;
...
void StoreMInstruction::output()
{
    ...
    if(use_list[0]->getReg() == 33){       //reg为33的说明这是超出4个的参数，不输出
        return;
    }
    if(se){
        int offset=dynamic_cast<TemporarySymbolEntry*>(se)->getOffset();
...
std::vector<MachineOperand*> MachineFunction::getSavedRegs() {
    ...
    for(auto regI : saved_regs){
        auto reg = new MachineOperand(MachineOperand::REG, regI);
        regs.push_back(reg);
    }
    return regs;
}
...
void MachineFunction::output()
{
    ...
    *  4. Allocate stack space for local variable */
    ...
    int spillOffset = (saved_regs.size() + 2)*4;
