...
void Instruction::replaceUse(Operand* oldUse, Operand* newUse){
    auto usePtr = std::find(operands.begin(),operands.end(), oldUse);
    (*usePtr)->removeUse(this);
    (*usePtr) = newUse;
    (*usePtr)->addUse(this);
...
BinaryInstruction::BinaryInstruction(unsigned opcode, Operand *dst, Operand *src1, Operand *src2, BasicBlock *insert_bb) : Instruction(BINARY, insert_bb)
{
    ...
    dst->setDef(this);
    src1->addUse(this);
    // if(src1->getDef()->isLoad()){
    //     std::cout<<src1->toStr()<<std::endl;
    // }
    src2->addUse(this);
    // if(src2->getDef()->isLoad()){
    //     std::cout<<src1->toStr()<<std::endl;
    // }
}
...
void AllocaInstruction::clear(){
    operands[0]->setDef(nullptr);
    // operands[0] = nullptr;
    operands[0] = new Operand(new TemporarySymbolEntry(TypeSystem::intType, SymbolTable::getLabel()));
    //新建一个操作数防止出现null
}
...
MachineOperand* Instruction::genMachineOperand(Operand* ope, bool isfp)
{
    auto se = ope->getEntry();
    MachineOperand* mope = nullptr;
    if(se->isConstant()){
        mope = new MachineOperand(MachineOperand::IMM, dynamic_cast<ConstantSymbolEntry*>(se)->getValue(),isfp);
        ...
        mope = new MachineOperand(MachineOperand::VREG, idSe->getLabel(),isfp);
    }
    else if(se->isVariable())
    {
        ...
        if(id_se->isGlobal()){
            if(id_se->getType()->isFunc())
                mope = new MachineOperand(id_se->toStr().c_str(), isfp);
            else{
                mope = new MachineOperand(id_se->toStr().c_str()+1, isfp);
            }                
        }else if(id_se->isParam()){//需要判断isParam的情况，此时可分配寄存器有限
            if (id_se->getParamNo() < 4){
                mope = new MachineOperand(MachineOperand::REG, id_se->getParamNo()+(isfp?16:0), isfp);
            }
            else{
                mope = new MachineOperand(MachineOperand::REG, -1, isfp); // 不会用到这个寄存器
            }
        }else
            exit(0);
    }
...
MachineOperand* Instruction::genMachineReg(int reg, bool isfp) 
{
    return new MachineOperand(MachineOperand::REG, reg, isfp);
}

MachineOperand* Instruction::genMachineVReg(bool isfp) 
{
    return new MachineOperand(MachineOperand::VREG, SymbolTable::getLabel(), isfp);
}

MachineOperand* Instruction::genMachineImm(double val, bool isfp) 
{
    return new MachineOperand(MachineOperand::IMM, val, isfp);
}

MachineOperand* Instruction::genMachineLabel(int block_no, bool isfp)
{
    std::ostringstream buf;
    buf << ".L" << block_no;
    std::string label = buf.str();
    return new MachineOperand(label, isfp);
}
...
void LoadInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    bool vldr = operands[0]->getType()->isFloat();
    if(operands[1]->getEntry()->isVariable()
    && dynamic_cast<IdentifierSymbolEntry*>(operands[1]->getEntry())->isGlobal())
    {
        auto dst = genMachineOperand(operands[0],vldr);
        ...
        // example: load r1, [r0]
        cur_inst = new LoadMInstruction(cur_block, dst, internal_reg2, nullptr, MachineInstruction::NONE, vldr);
        cur_block->InsertInst(cur_inst);
    }
    // Load local operand
    ...
    && operands[1]->getDef()->isAlloc())
    {
        ...
        //     src2=internal_reg1;
        // }
        cur_inst = new LoadMInstruction(cur_block, dst, operands[1]->getEntry(), MachineInstruction::NONE, vldr);
        cur_block->InsertInst(cur_inst);
    }
    ...
    else
    {
        ...
        dynamic_cast<LoadMInstruction*>(cur_inst)->setVldr(vldr);
        cur_block->InsertInst(cur_inst);
    }
}
...
void StoreInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    auto cur_block = builder->getBlock();
    MachineInstruction* cur_inst = nullptr;
    bool vstr = operands[1]->getType()->isFloat()/*||operands[0]->getType()->isFloat()*/;
    auto dst = genMachineOperand(operands[0]);
    auto src = genMachineOperand(operands[1], vstr);
    int NONE = MachineInstruction::NONE;
    if (src->isImm()) {
        auto internal_reg = genMachineVReg();
        cur_inst = new LoadMInstruction(cur_block, internal_reg, src);
        cur_block->InsertInst(cur_inst);
        if(vstr){
            auto s_reg = genMachineVReg(true);
            cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, s_reg, internal_reg);
            cur_block->InsertInst(cur_inst);
            src = s_reg;
        }else{
    ...
    {
        auto inter_reg = genMachineVReg();
        // example: load r0, addr_a
        cur_inst = new LoadMInstruction(cur_block, inter_reg, dst);
        cur_block->InsertInst(cur_inst);
        // example: store r1, [r0]
        cur_inst = new StoreMInstruction(cur_block, src, inter_reg, nullptr, NONE, vstr);
        cur_block->InsertInst(cur_inst);
    }
            ...
            && operands[0]->getDef()->isAlloc())
    {
        ...
        cur_inst = new StoreMInstruction(cur_block, src, operands[0]->getEntry(),isparam, NONE, vstr);
        cur_block->InsertInst(cur_inst);
    }
    // store to pointer   待改
    else if (operands[0]->getType()->isPTR()) {
        cur_inst = new StoreMInstruction(cur_block, src, dst, nullptr, NONE, vstr);
        cur_block->InsertInst(cur_inst);
    }
}
...
void BinaryInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    auto cur_block = builder->getBlock();
    bool isFloat = operands[0]->getType()->isFloat();
    auto dst = genMachineOperand(operands[0], isFloat);
    auto src1 = genMachineOperand(operands[1], operands[1]->getType()->isFloat());
    auto src2 = genMachineOperand(operands[2], operands[2]->getType()->isFloat());
    /* HINT:
    ...
    * As to other instructions, such as MUL, CMP, you need to deal with this situation, too.*/
    ...
    if(src1->isImm()){
        ...
        cur_block->InsertInst(cur_inst);
        if(src1->isfp){
            src1 = genMachineVReg(src1->isfp);
            cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV,src1, temp_reg);
            cur_block->InsertInst(cur_inst);
        }else
            src1 = temp_reg;
    }
    if(src2->isImm()){
        //考虑不合法立即数的情况
        double imm_val = src2->getVal();
        //float还没考虑
        if(src2->isfp||(opcode==MUL||opcode==DIV||opcode==MOD)||(imm_val>255||imm_val<-255)){
            auto temp_reg = genMachineVReg();
            cur_inst = new LoadMInstruction(cur_block, temp_reg, src2);
            cur_block->InsertInst(cur_inst);
            if(src2->isfp){
                src2 = genMachineVReg(src2->isfp);
                cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV,src2, temp_reg);
                cur_block->InsertInst(cur_inst);
            }
            else
                src2 = temp_reg;
...
void RetInstruction::genMachineCode(AsmBuilder* builder)
{
    ...
    if (!operands.empty()) {
        if(operands[0]->getType()->isFloat()){
            auto dst = genMachineReg(16, true);
            auto src = genMachineOperand(operands[0], true);
            if(src->isImm()){
                auto temp_reg = genMachineVReg();
                auto cur_inst = new LoadMInstruction(cur_block, temp_reg, src);
                cur_block->InsertInst(cur_inst);
                src = temp_reg;
            }
            auto cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, dst, src);
            cur_block->InsertInst(cur_inst);
        }else{
...
void CastInstruction::genMachineCode(AsmBuilder* builder){
    ...
    auto cur_block = builder->getBlock();
    auto dst = genMachineOperand(operands[1], operands[1]->getType()->isFloat());
    auto src = genMachineOperand(operands[0], operands[0]->getType()->isFloat());
    MachineInstruction* cur_inst;
    if(opcode==ITOF){
        dst->isfp = true;
        if(src->isImm()){
            auto load_reg = genMachineVReg();
            cur_inst = new LoadMInstruction(cur_block, load_reg, src);
            cur_block->InsertInst(cur_inst);
            src = load_reg;
        }
        auto mov_reg = genMachineVReg(true);
        cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, mov_reg ,src);
        cur_block->InsertInst(cur_inst);
        cur_inst = new VcvtMInstruction(cur_block, VcvtMInstruction::ITOF, dst, mov_reg);
        cur_block->InsertInst(cur_inst);
    }else if(opcode==FTOI){
        src->isfp = true;
        if(src->isImm()){
            ...
            cur_block->InsertInst(cur_inst);
            auto mov_reg = genMachineVReg(true);
            cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, mov_reg, load_reg);
            cur_block->InsertInst(cur_inst);
            src = mov_reg; 
        }
        auto ftoi_dst = genMachineVReg(true);
        cur_inst = new VcvtMInstruction(cur_block, VcvtMInstruction::FTOI, ftoi_dst, src);
        cur_block->InsertInst(cur_inst);
        cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, dst, ftoi_dst);
        cur_block->InsertInst(cur_inst);
    }else if(opcode==BTOI){
...
void CallInstruction::genMachineCode(AsmBuilder* builder){
    auto cur_block = builder->getBlock();
    MachineInstruction* cur_inst;
    for(int i=1; i< operands.size()&&i<5;i++){      //float还没考虑 //2023年1月1日17:46:10 add float
        bool isfp = operands[i]->getType()->isFloat();
        auto mv_src = genMachineOperand(operands[i], isfp);
        auto mv_dst = genMachineReg(isfp?i+15:i-1, isfp);
        if (mv_src->isImm()) {    //认为不合法的立即数采用ldr指令
            if(isfp){
                auto temp_reg = genMachineVReg();
                cur_inst = new LoadMInstruction(cur_block, temp_reg, mv_src);
                cur_block->InsertInst(cur_inst);
                cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, mv_dst, temp_reg);
            }else if(mv_src->getVal() > 255)
            ...
            if(isfp)
                cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, mv_dst, mv_src);
            else
                cur_inst = new MovMInstruction(cur_block, MovMInstruction::MOV,mv_dst, mv_src);
        }
        cur_block->InsertInst(cur_inst);
    ...
    for(int i= operands.size()-1; i>4; i--){    //push序号大于4的实参
        bool isfp = operands[i]->getType()->isFloat();
        auto push_op = genMachineOperand(operands[i], isfp);
        if(push_op->isImm()||push_op->isfp){
            auto internal_op = genMachineVReg(push_op->isfp);
            if(push_op->isfp){
                if(push_op->isImm()){
                    auto temp_reg = genMachineVReg();
                    cur_inst = new LoadMInstruction(cur_block, temp_reg, push_op);
                    cur_block->InsertInst(cur_inst);
                    push_op = temp_reg;
                }
                cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, internal_op, push_op);
                cur_block->InsertInst(cur_inst);
                push_op = internal_op;
            }else{
        ...
        if(push_op->isfp)
            cur_inst = new StackMInstrcuton(cur_block, StackMInstrcuton::VPUSH, push_op);
        else
    ...
    if (operands[0]) {
        if(operands[0]->getType()->isFloat()){
            auto ret_op = genMachineOperand(operands[0], true);
            auto s0 = new MachineOperand(MachineOperand::REG, 16,true);//s0
            cur_inst = new MovMInstruction(cur_block, MovMInstruction::VMOV, ret_op, s0);
            cur_block->InsertInst(cur_inst);
        }else{
...
void FCmpInstruction::genMachineCode(AsmBuilder* builder){
    ...
    cur_inst = new CmpMInstruction(cur_block, src1, src2, opcode, true);
    cur_block->InsertInst(cur_inst);
    builder->setCmpOpcode(opcode);
    // for(auto it = operands[0]->use_begin();it != operands[0]->use_end();it++){
    //     std::cout<<(*it)->isCond()<<std::endl;
...
PhiInstruction::PhiInstruction(Operand* dst, BasicBlock* insert_bb): Instruction(Instruction::PHI,insert_bb){
    operands.push_back(dst);
    dst->setDef(this);
    this->dst = dst;
}

void PhiInstruction::addIncoming(Operand* inOp, BasicBlock* inBlock){
    inBlocks.push_back(inBlock);
    srcs[inBlock] = inOp;
}
