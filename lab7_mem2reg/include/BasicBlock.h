...
public:
    ...
    void insertCopyIns(Instruction*);
    void remove(Instruction *);
    void removeAfter(Instruction *);
    ...
    int index;
    /*mem2reg*/
    std::set<BasicBlock*> dominFrontierVec;
    void addDomFrontier(BasicBlock* block){
        dominFrontierVec.insert(block);
    }
    void insertPhiNode(Operand* op){
        auto ins = new PhiInstruction(op);
        insertFront(ins);
    }
    void showDF(){
        for(auto df : dominFrontierVec)
            std::cout<<index<<":"<<df->index<<std::endl;
    }
    /*end*/
};
