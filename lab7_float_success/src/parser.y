...
ConstDecl
    : CONST Type {
        ...
        for(auto it = constEntries->begin();it != constEntries->end();it++){
            IdentifierSymbolEntry *se = dynamic_cast<IdentifierSymbolEntry *>(*it);
            // if(se->isGlobal())
            // {
            //     std::cout<<se->toStr()<<" pa_val:"<<se->getValue()<<std::endl;
            // }
        }
...
ConstDef 
    ...
    | ID ASSIGN ConstInitVal {          //2022年11月5日11:53:58 add by zsr
        ...
        // std::cout<<"val: "<<se->getValue()<<std::endl;
        identifiers->install(nowId, se);
        constEntries->push_back(se);
...
FuncDef
    ...
    BlockStmt
    {
        ...
        delete top;
        delete []$2;
        IdentifierSymbolEntry::paramNum = 0;
    }
    ;
FuncFParams
    : FuncFParam {
