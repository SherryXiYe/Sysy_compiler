...
public:
    ...
    MachineOperand(int tp, double val, bool isfp = false);
    MachineOperand(std::string label, bool isfp = false);
...
protected:
    ...
    void PrintCond();
    enum instType { BINARY, LOAD, STORE, MOV, BRANCH, CMP, STACK, CVT};
public:
    ...
    MachineBlock* getBlock(){return parent;}
};
...
public:
    LoadMInstruction(MachineBlock* p,
                    ...
                    int cond = MachineInstruction::NONE, bool vldr = false);
    LoadMInstruction(MachineBlock* p,
                    MachineOperand* dst, SymbolEntry* se,int cond = MachineInstruction::NONE, bool vldr= false);
    void setVldr(bool vldr){this->vldr = vldr;};
    void output();
...
public:
    StoreMInstruction(MachineBlock* p,
                    ...
                    int cond = MachineInstruction::NONE, bool vstr=false);
    StoreMInstruction(MachineBlock* p,
                    MachineOperand* src1, SymbolEntry* se, bool isParam, int cond = MachineInstruction::NONE, bool vstr=false);
    void output();
    SymbolEntry* getSymPtr(){return se;};
    void setVstr(bool vstr){this->vstr = vstr;}
};

class MovMInstruction : public MachineInstruction
{
public:
    enum opType { MOV, MVN, VMOV/*mov float*/ };
...
public:
    ...
    CmpMInstruction(MachineBlock* p, 
                ...
                int cond = MachineInstruction::NONE, bool vcmp =false);
    void output();
};
...
{
public:
    enum opType { PUSH, POP, VPUSH, VPOP/*float*/ };
...
public:
    ...
    void insertBefore(MachineInstruction* dstInst, MachineInstruction* insertInst){
        auto dst_pos = std::find(inst_list.begin(), inst_list.end(), dstInst);
        inst_list.insert(dst_pos, insertInst);
    }
    void insertAfter(MachineInstruction* dstInst, MachineInstruction* insertInst){
        auto dst_pos = std::find(inst_list.begin(), inst_list.end(), dstInst);
        dst_pos++;
        inst_list.insert(dst_pos, insertInst);
    }
...
public:
    ...
    * you can use this function in LinearScan::genSpillCode() */
    ...
    void addSavedSRegs(int regno) {saved_sregs.insert(regno);};
    std::vector<MachineOperand*> getSavedRegs();
    std::vector<MachineOperand*> getSavedSRegs();
    void output();
    MachineUnit* getUnit(){return parent;}
};
...
public:
    ...
    void output(); 
    void addGlobalList(SymbolEntry* id_se){ global_list.push_back(id_se);}; 
};
